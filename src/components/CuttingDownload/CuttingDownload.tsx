...
import { downloadFile1DCutting, downloadFile2DCutting } from '../../functions/fetchFiles';
...
export const CuttingDownload = ({
    ...
    const handlerDownloadPDF = async () => {
        if (isICalculate1D(dataCalculate)) {
            ...
            await downloadFile2DCutting(
                JSON.stringify(dataCalculate),
                'pdf',
    ...
    return (
        <Flex vertical className={styles['cutting-1D-download']}>
            ...
            {is2D ? (
                <Button onClick={() => handlerDownloadDXF()}>Скачать схему dxf</Button>
            ) : (
                <Button onClick={() => handlerDownloadCSV()}>Скачать схему csv</Button>
            )}
        </Flex>
    );
};
